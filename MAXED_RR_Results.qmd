...
+--------------------------------------------------------------+--------------------------------------------------------------------------------------------------------------------------------+
| Heterogeneity of Variance across Groups in Exercise Response | DV \~ 1 + Age + Group + Time + Time x Group,                                                                                   |
|                                                              |                                                                                                                                |
| (using `nlme`)                                               | random = list(id = pdDiag(form = \~ Group)),                                                                                   |
|                                                              |                                                                                                                                |
...
Visual depiction of raw changes in affect measurements during exercise across groups is presented in @fig-AffectLoessPlot. Average levels of change from 0 to 30 minutes along with variance in affect across group and condition are presented in @fig-AffectChange, and all specific values for average levels of change across groups (30 min vs. BL), maximum increases and maximum decreases from baseline, and variance are presented in Supplemental @tbl-affectChange. Patterns of results indicate that individuals in the ED group consistently feel more positive and less negative emotion increases across the 30 mins of exercise. Change indicators suggest (ED vs. HC Cohen's d \> .2) that individuals with EDs may feel less 'crummy' and more 'calm', relative to controls, during exercise. Fatigue increases in the ED group also appear delayed, relative to HC.
...
Model fit indices for each variable and task are presented in Supplmental @tbl-AffectModFit.
...
From an interpretation of the estimated marginal means, there are initial indications of ED and Control Groups responding differently to tasks. For instance, the ED group exhibited higher negative affect (feeling crummy) in the rest condition of the self-paced task (i.e. when asked to ingest a milkshake and then rest), which may to be mitigated by the opportunity to exercise. The ED group also showed a slower increase in fatigue, relative to the Control group, in both Prescribed and Self-paced Exercise, and a trend toward increased enthusiasm during Prescribed Exercise, relative to a decrease in Enthusiasm in the control group. These potential trends are suggested by graphical depictions; however, the current pilot trial was not sufficiently powered to detect three-way interactions, and no significant Group x Time x Condition interactions emerged in the multi-level model, suggesting that these are likely to be small-to-moderate (rather than large) effects, if they do exist.

Evaluations of the between and within subject variance across groups via introducing additional random effects and comparing nested models indicated potentially higher within subject variance in fatigue and calmenss during prescribed exercise and fatigue and crumminess during self-paced exercise among the ED group, relative to the HC group (see Supplemental @tbl-affectHOV).

#### Within ED Group Model

Model fit indices for each variable and task are presented in Supplmental @tbl-AffectModFit. Marginal $R^2$ values indicate that fixed effects specified in the current models are, together, accounting for `r round(min(ed_affect_models$R2.Marginal)*100,0)`-`r round(max(ed_affect_models$R2.Marginal)*100,0)`% of variance in affect during the course of the task, depending on task and variable. Results within the ED group are presented in Supplemental @tbl-affectED. A Time x Condition effect emerged for calmness during the Prescribed exercise task and fatigue during the Self-Paced exercise task. Individuals with EDs reported feeling less calm and more fatigued over time during exercise vs. rest. Futher, older age was associated with feeling less crummy during the Self-Paced task and less Fatigued during the Prescribed task. There were no BMI or BMI x Condition effects, indicating limited impact of BMI on affect during exercise among the ED group.

### Body Image

#### Descriptives

Visual depiction of raw changes in body image changes during exercise across groups is presented in @fig-BissLoessPlot. Average levels of change from 0 to 30 minutes and variance in body image across groups are presented in @fig-BISSChange. All specific values for average levels of change across groups (30 min vs. BL), maximum increases and maximum decreases from baseline, and variance are presented in Supplemental @tbl-BISSChange. Patterns of results indicate that individuals in the ED group consistently feel worse about their bodies, although exercise does have some impact on body image. In all measures, individuals with EDs had greater improvements in body image during exericse, as compared to controls. Change indicators suggest (ED vs. HC Cohen's d \> .2) that individuals with EDs specifically feel better about their weight, looks, and appearance, as well as feeling better on average and feeling better than the average person after self-paced exercise and better about their weight after prescribed exercise.
...
knitr::include_graphics('figs/3.body_image/biss_variance_exercise.png')
...
Model fit indices for each variable and task are presented in Supplmental @tbl-BISSModFit. Marginal $R^2$ values indicate that fixed effects specified in the current models are, together, accounting for `r round(min(overall_biss_models$R2.Marginal)*100,0)`-`r round(max(overall_biss_models$R2.Marginal)*100,0)`% of variance in body image during exercise across variable and task. Results from each variable in the overall model (see @tbl-params) are presented in Supplemental @tbl-bodyimage with estimated marginal means for each timepoint presented in @fig-bissemm. Group was a significant predictor of all body image items -- with the HC group feeling better about their body, relative to the ED group. Higher BMI was assocaited with feeling less attractive than the average person, but there were no significant BMIxCondition effects, indicating that exercise did not influence this effect. There was an overall time effect several items during the Self-Paced condition, with individuals generally feeling better over time. There was also a Group x Time interaction for the BISS average during prescribed exercise, such that individuals in the ED group had greater increases in average BISS scores relative to controls across both exercise and rest. Two Time x Condition x Group interactions emerged. Coupled with an interpretation of marginal means, these interactions indicate that the ED group had more positive increases in feelings about their weight and overall body image during self-paced exercise, relative to the HC group. Evaluations of variance across the two groups also indicates more within person variance in the ED group, as compared to the HC group, on most BISS measurements during exercise (see Supplemental @tbl-bissHOV).
...
Differences between ED and Control on Pre, Post- Raw Change (Post-Pre) and residualized change scores in biomarkers post-exercise are presented in @fig-forest. Full t-test results across both exercise and rest conditions along with tests of heterogeneity of variance in exercise condition response across the ED and Control group using each outcome approach are presented in Supplementary @tbl-BiomarkerChange.

#### **Within ED Group**

Differences between ED and Control on Pre, Post- Raw Change (Post-Pre) and residualized change scores in biomarkers post-exercise are presented in @fig-forest, and full t-test results across both exercise and rest conditions using each outcome approach are presented in Supplementary @tbl-BiomarkerChangeED. \# KS NOTE: Not putting any text here yet to explain graphs since we'll have more biomarker data incoming \##
...
# filter out biomarker variables for now
biomarkers <- c('Cortisol_RawChange', 'BDNF_RawChange', 'Leptin_RawChange', 'Cortisol_ResidChange', 'BDNF_ResidChange', 'Leptin_ResidChange')
cor_ci_results_ed_nobio <- cor_ci_results_ed[!cor_ci_results_ed$var1 %in% biomarkers & !cor_ci_results_ed$var2 %in% biomarkers,]

cor_ci_results_control_nobio <- cor_ci_results_control[!cor_ci_results_control$var1 %in% biomarkers & !cor_ci_results_control$var2 %in% biomarkers,]

# combine the two dataframes
# add a column 'ED' to the ED dataframe, 'Control' to the control dataframe
...
Correlations between acute exercise response variables are presented in @fig-exresponse, and full correlation data with 95% CIs is presented in @tbl-exresponsecorrs. Average zero-order $r^2$ between exercise response variables in the control group is `r round(mean(cor_ci_results_control_nobio$estimate*2, na.rm = TRUE), 2)` and in the ED group is `r round(mean(cor_ci_results_ed_nobio$estimate*2, na.rm = TRUE), 2)`, indicating stronger association between exercise response variables in the ED group, relative to the control group.
...
Correlations between exercise response variables with key variables of interest are presented in @fig-exresponse2. Full correlation data with 95% CIs is presented in @tbl-exresponsecorrs2. *For only those in the ED group only, greater body image response to acute exercise was associated with higher Self-Paced exercise intensity, higher self-reported drive for muscularity, and lower body mass index. Increases in feeling fatigued and crummy during Prescribed exercise were associated with drive for muscularity and compulsive exercise only for the ED group (all Pearson rs \> 0.25).*
...
All supplemental tables are available in the [Supplemental Tables Excel Workbook](Supplemental_Tables.xlsx). Supplemental tables and figures are also presented below
...
wb <- createWorkbook()
addWorksheet(wb, "ST1-Affect_Descriptives")
writeData(wb, "ST1-Affect_Descriptives", affect_descriptives, rowNames = FALSE)

# Add more sheets for other dataframes
addWorksheet(wb, "ST2-Affect_Models")
writeData(wb, "ST2-Affect_Models", affect_models_summary, rowNames = FALSE)

addWorksheet(wb, "ST3-Affect_ModelFit_Indices")
writeData(wb, "ST3-Affect_ModelFit_Indices", AMFI, rowNames = FALSE)

addWorksheet(wb, "ST4-Affect_HOV")
writeData(wb, "ST4-Affect_HOV", affect_hov_results, rowNames = FALSE)

addWorksheet(wb, "ST5-Affect_Models_ED")
writeData(wb, "ST5-Affect_Models_ED", affect_models_ed_summary, rowNames = FALSE)

addWorksheet(wb, "ST6-BISS_Models")
writeData(wb, "ST6-BISS_Models", biss_models_summary, rowNames = FALSE)

addWorksheet(wb, "ST7-BISS_ModelFit_Indices")
writeData(wb, "ST7-BISS_ModelFit_Indices", BMFI, rowNames = FALSE)

addWorksheet(wb, "ST8-BISS_HOV")
writeData(wb, "ST8-BISS_HOV", biss_hov_results, rowNames = FALSE)

addWorksheet(wb, "ST9-BISS_Models_ED")
writeData(wb, "ST9-BISS_Models_ED", biss_models_ed_summary, rowNames = FALSE)

addWorksheet(wb, "ST10-BiomarkerChange")
writeData(wb, "ST10-BiomarkerChange", supplement_ttable, rowNames = FALSE)

addWorksheet(wb, "ST11-BiomarkerChange_ED")
writeData(wb, "ST11-BiomarkerChange_ED", ed_biomarkers, rowNames = FALSE)

addWorksheet(wb, "ST12-ExResponseCorrelations")
writeData(wb, "ST12-ExResponseCorrelations", cor_ci_results_wide, rowNames = FALSE)
