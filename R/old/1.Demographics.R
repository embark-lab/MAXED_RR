...
Redcap_files <- list.files('data/RedCap')
Redcap_wide <- Redcap_files[grepl("wide", Redcap_files)]

load('data/RedCap/redcap_raw_enrolled.RData')
load(paste0("data/RedCap/", Redcap_wide))
...
# bar chart showing percentage under 18
ggplot(u18, aes(x = as.factor(u18), y = n, fill = as.factor(u18))) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values = c("#fc6d46", "#1a4e66")) +
  theme_minimal() +
