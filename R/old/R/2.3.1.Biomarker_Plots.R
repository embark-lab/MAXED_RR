...
Assay_results_long <- Assay_results %>%
  pivot_longer(cols = c(BDNF, Leptin), names_to = 'Assay', values_to = 'Value') %>%
  mutate(Assay = factor(Assay, levels = c('BDNF', 'Leptin'))) |> 
...
Assay_results_long %>%
  ...
  # rename 'group_factor' to 'Group' in legend
  # add embark colors to colors
...
Mean_assay_plot <- ggplot(assay_summary_data, aes(x = Time, group = interaction(group_factor, Condition))) +
...
# adjust the y position 
  mutate(y_pos = case_when(position_factor == group_factor ~ midpoint*1.03,
                         position_factor != group_factor ~ midpoint*0.97)) 

# Annotate Mean Assay Plot with Cohen's d values
Mean_assay_plot + 
  geom_text(data = annotate.3, aes(label = paste('Ex d =', round(cohens_d, 2)), x = 1.3, y = y_pos), 
